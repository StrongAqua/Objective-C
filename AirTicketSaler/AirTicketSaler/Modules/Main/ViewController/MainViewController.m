...
- (void)presentFirstViewControllerIfNeeded
{
    ...
    if (!isFirstStart) {
        ...
        firstViewController.modalPresentationStyle =
              ...
              [[APIManager sharedInstance] ticketsWithRequest:self->_searchRequest withCompletion:^(NSArray *tickets) {
                  [[ProgressView sharedInstance] dismiss:^{
                      if (tickets.count > 0) {
                          ...
                          UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@":(" message:[@"tickets_not_found" localize] preferredStyle: UIAlertControllerStyleAlert];
